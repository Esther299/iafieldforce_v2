...
import { contextualTips } from "@/shared/data";
...
function detectEscalationDept(query: string): EscalationDept {
  ...
  if (/regulator|cofepris|registro|etiqueta|ipp oficial/.test(q))
    return "regulatorio";
  if (/precio|descuento|bonific|comercial|competencia de mercado/.test(q))
    return "comercial";
  return "medico";
}
...
function fillTemplate(
  ...
  return tpl
    ...
    .replaceAll(
      "{realRepName}",
      ctx.realRepName ?? "su visitador de territorio",
    )
    .replace(/\s+/g, " ")
    .trim();
}
...
export function buildContextualOpener(
  ...
  if (doctor.birthday === "08-12" || notes.some((n) => /cumplea/i.test(n))) {
    return fillTemplate(
      contextualTips.find((t) => t.type === "birthday")!.template,
      ctx,
    );
  }
  if (notes.some((n) => /tr[aá]fico/i.test(n))) {
    return fillTemplate(
      contextualTips.find((t) => t.type === "traffic")!.template,
      ctx,
    );
  }
  if (notes.some((n) => /temperatura|fr[ií]o|clima/i.test(n))) {
    return fillTemplate(
      contextualTips.find((t) => t.type === "weather")!.template,
      ctx,
    );
  }
  return fillTemplate(
    contextualTips.find((t) => t.type === "event")!.template,
    ctx,
  );
}
...
export function generateGovernedReply(
  userText: string,
  ctx: ReplyContext,
): GovernedReply {
  ...
  const firstName =
    ctx.pharmacy?.name.split(" ")[0] ?? ctx.doctor?.name.split(" ")[0] ?? "";
  const tplCtx = {
    title,
    lastName,
    firstName,
  ...
  const wantsEvidence =
    /estudio|evidencia|cl[ií]nico|resultado|eficacia|dato/.test(q);
  const wantsProduct =
    /producto|presenta|indicaci[oó]n|qu[eé] es|para qu[eé]|perfil|posolog/.test(
      q,
    );
  const wantsSample = /muestra|sample|enviar|despach/.test(q);
  const greeting =
    /^(hola|buenos|buenas|saludos|hey)\b/.test(q) || q.length < 12;
  ...
  if (greeting || ctx.history.filter((m) => m.role === "user").length === 0) {
    ...
    const support =
      ctx.audience === "covered_doctors" &&
      ctx.campaign.script.supportRepMention &&
      ctx.realRep
    ...
    return {
      content: body,
      sources: [
        "Script de campaña aprobado",
        ...productIds.map((id) => `Producto ${id}`),
      ],
      escalated: false,
      ctaShown: false,
      governanceNotes: notes,
