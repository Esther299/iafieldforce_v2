...
export function Dashboard() {
  ...
  return (
    <div className="animate-fade-up space-y-6">
      ...
      {isSalesForceCreator && (
        <Card className="border-brand-200 bg-gradient-to-br from-brand-50 via-white to-emerald-50/50 p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-600">
                Región 1-4 consolidada. 18 reps activos, 92% de cobertura logística y 6 campañas en validación.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-brand-200 bg-white/80 px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.16em] text-ink-400">Reps activos</p>
                <p className="mt-1 text-2xl font-black text-brand-800">18</p>
              </div>
              <div className="rounded-2xl border border-brand-200 bg-white/80 px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.16em] text-ink-400">Cobertura</p>
                <p className="mt-1 text-2xl font-black text-brand-800">92%</p>
              </div>
              <div className="rounded-2xl border border-brand-200 bg-white/80 px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.16em] text-ink-400">Campañas</p>
                <p className="mt-1 text-2xl font-black text-brand-800">6</p>
              </div>
            </div>
          </div>
        </Card>
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-100/60 px-2 py-0.5 rounded-full">
                {isSalesForceCreator ? 'Resumen operativo' : 'Resumen Financiero'}
              </span>
              <span className="text-xs text-ink-400 hidden sm:inline">
                · {isSalesForceCreator ? 'Cobertura y fuerza de ventas' : 'Clic para abrir métricas de cobertura y saldo'}
              </span>
            </div>
            <h3 className="text-base font-bold text-ink-900 mt-0.5">
              {isSalesForceCreator ? 'Estado rápido, Cobertura & Fuerza de ventas' : 'Estado rápido, Cobertura & Saldo'}
            </h3>
          </div>
        </div>
